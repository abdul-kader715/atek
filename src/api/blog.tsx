
/* Image */
import img1 from "../img/blog/blog_1_1.jpg";
import img2 from "../img/blog/blog_1_2.jpg";
import img3 from "../img/blog/blog_1_3.jpg";
import img5 from "../img/blog/blog_3_1.jpg";
import img6 from "../img/blog/blog_3_2.jpg";
import img7 from "../img/blog/blog_3_3.jpg";
import img8 from "../img/blog/blog_5_1.jpg";
import img9 from "../img/blog/blog_5_2.jpg";
import img10 from "../img/blog/blog_5_3.jpg";

import grid1 from "../img/blog/blog_1_3.jpg";

import bloggrig1 from "../img/blog/blog_2_1.jpg";
import bloggrig2 from "../img/blog/blog_2_2.jpg";
import bloggrig3 from "../img/blog/blog_2_3.jpg";

import blog_7_1 from "../img/blog/blog_7_1.jpg";
import blog_7_2 from "../img/blog/blog_7_2.jpg";
import blog_7_3 from "../img/blog/blog_7_3.jpg";

import blog_8_1 from "../img/blog/blog_8_1.jpg";
import blog_8_2 from "../img/blog/blog_8_2.jpg";
import blog_8_3 from "../img/blog/blog_8_3.jpg";

import bloggrig91 from "../img/blog/blog_9_1.jpg";
import bloggrig92 from "../img/blog/blog_9_2.jpg";
import bloggrig93 from "../img/blog/blog_9_3.jpg";

import blog_12_1 from "../img/blog/blog_12_1.jpg";
import blog_12_2 from "../img/blog/blog_12_2.jpg";
import blog_12_3 from "../img/blog/blog_12_3.jpg";

import blog_10_1 from "../img/blog/blog_10_1.jpg";
import blog_10_2 from "../img/blog/blog_10_2.jpg";
import blog_10_3 from "../img/blog/blog_10_3.jpg";

import blog_13_1 from "../img/blog/blog_13_1.jpg";
import blog_13_2 from "../img/blog/blog_13_2.jpg";
import blog_13_3 from "../img/blog/blog_13_3.jpg";
import blog_14_1 from "../img/blog/blog_14_1.jpg";
import blog_14_2 from "../img/blog/blog_14_2.jpg";
import blog_14_3 from "../img/blog/blog_14_3.jpg";


const BlogData = [

   {
      id: 1,
      imageUrl: img1,
      grid: grid1,
      urlGrid: bloggrig1,
      urlGrid9: bloggrig91,
      urlGrid12: blog_12_1,
      urlGrid13: blog_10_1,
      gridPage: img1,
      date: 'July 05, 2025',
      minet: '08 min read',
      title: 'Top 10 IT Solutions Every Business Needs in 2025',
      title2: 'The Impact of AI and Machine Learning on IT Solutions',
      title9: 'Simplifying Complexity with Web & App Excellence.',
      descpt: 'Data Analytics is revolutionizing industries by enabling smarter decisions, optimizing business operations',
      slug: '1',
   },
   {
      id: 2,
      imageUrl: img2,
      grid: grid1,
      urlGrid: bloggrig2,
      urlGrid9: bloggrig92,
      urlGrid12: blog_12_2,
      urlGrid13: blog_10_2,
      gridPage: img2,
      date: 'August 15, 2025',
      minet: '08 min read',
      title: 'Exploring the Benefits of End-to-End IT Solution Services',
      title2: 'Top 10 IT Solutions Every Business Needs in 2025',
      title9: 'Pioneering Technology to Elevate Your Brand.',
      descpt: 'Data Analytics is revolutionizing industries by enabling smarter decisions, optimizing business operations',
      slug: '2',
   },
   {
      id: 3,
      imageUrl: img3,
      grid: grid1,
      urlGrid: bloggrig3,
      urlGrid9: bloggrig93,
      urlGrid12: blog_12_3,
      urlGrid13: blog_10_3,
      gridPage: img3,
      date: 'Sep 15, 2025',
      minet: '08 min read',
      title: 'The Impact of AI and Machine Learning on IT Solutions',
      title2: 'Exploring the Benefits of End-to-End IT Solution Services',
      title9: 'Building Innovative Solutions for a Connected World.',
      descpt: 'Data Analytics is revolutionizing industries by enabling smarter decisions, optimizing business operations',
      slug: '3',
   },
   {
      id: 4,
      imageUrl: bloggrig3,
      imageUrl7: blog_7_1,
      imageUrl8: blog_8_1,
      urlGrid9: bloggrig91,
      urlGrid12: blog_12_1,
      grid: grid1,
      urlGrid: img2,
      urlGrid13: blog_10_2,
      gridPage: blog_13_1,
      date: 'Sep 15, 2025',
      minet: '08 min read',
      title: 'The Impact of AI and Machine Learning on IT Solutions',
      title2: 'Top 10 IT Solutions Every Business Needs in 2025',
      title7: 'Where Tech Meets Support and Solutions Begin.',
      title8: 'Top Trends Shaping the Future of Software Development in 2025.',
      title9: 'Simplifying Complexity with Web & App Excellence.',
      descpt: 'Data Analytics is revolutionizing industries by enabling smarter decisions, optimizing business operations',
      slug: '4',
   },
   {
      id: 5,
      imageUrl: img5,
      imageUrl7: blog_7_2,
      imageUrl8: blog_8_2,
      urlGrid9: bloggrig92,
      urlGrid12: blog_12_2,
      grid: grid1,
      gridPage: blog_13_2,
      date: 'Sep 15, 2025',
      minet: '08 min read',
      title: 'What is Data Analytics? Everything You Need to Know',
      title7: 'Solving Tech Issues with Speed and Precision.',
      title8: 'The Role of UI/UX in Creating Software That Users Love',
      title9: 'Building Innovative Solutions for a Connected World.',
      descpt: 'Data Analytics is revolutionizing industries by enabling smarter decisions, optimizing business operations',
      slug: '5',
   },
   {
      id: 6,
      imageUrl: img6,
      imageUrl7: blog_7_3,
      imageUrl8: blog_8_3,
      urlGrid9: bloggrig93,
      urlGrid12: blog_12_3,
      grid: grid1,
      gridPage: blog_13_3,
      date: 'July 07, 2024',
      minet: '07 min read',
      title: 'Top 10 Real-World Applications of Machine Learning',
      title7: 'The First Call You Make for Tech Solutions.',
      title8: 'Cybersecurity Is a Top Priority in Modern Software Solutions',
      title9: 'Simplifying Complexity with Web & App Excellence.',
      descpt: 'Data Analytics is revolutionizing industries by enabling smarter decisions, optimizing business operations',
      slug: '6',
   },
   {
      id: 7,
      imageUrl: img7,
      grid: grid1,
      gridPage: blog_14_1,
      date: 'July 07, 2024',
      minet: '07 min read',
      title: 'How Machine Learning Algorithms Learn: A Deep Dive',
      slug: '7',
   },
   {
      id: 8,
      imageUrl: img8,
      grid: grid1,
      gridPage: blog_14_2,
      date: 'July 07, 2024',
      minet: '07 min read',
      title: 'The Security Risks of Changing Package Owners',
      slug: '8',
   },
   {
      id: 9,
      imageUrl: img9,
      grid: grid1,
      gridPage: blog_14_3,
      date: 'July 07, 2024',
      minet: '07 min read',
      title: 'The Security Risks of Changing Package Owners',
      slug: '9',
   },
   {
      id: 10,
      imageUrl: img10,
      grid: grid1,
      gridPage: blog_14_2,
      date: 'July 07, 2024',
      minet: '07 min read',
      title: 'The Security Risks of Changing Package Owners',
      slug: '10',
   },
   {
      id: 11,
      gridPage: blog_13_1,
      grid: grid1,
      date: 'July 09, 2025',
      minet: '09 min read',
      title: 'Top 10 IT Solutions Every Business Needs in 2025',
      slug: '11',
   },
   {
      id: 12,
      gridPage: blog_13_2,
      grid: grid1,
      date: 'January 10, 2025',
      minet: '10 min read',
      title: 'Exploring the Benefits of End-to-End IT Solution Services',
      slug: '12',
   },
   {
      id: 13,
      gridPage: blog_13_3,
      grid: grid1,
      date: 'November 12, 2025',
      minet: '11 min read',
      title: 'The Impact of AI and Machine Learning on IT Solutions',
      slug: '13',
   },
   {
      id: 14,
      gridPage: blog_14_1,
      grid: grid1,
      date: 'December 05, 2025',
      minet: '05 min read',
      title: 'Top 10 IT Solutions Every Business Needs in 2025',
      slug: '14',
   },
   {
      id: 15,
      gridPage: blog_14_2,
      grid: grid1,
      date: 'January 01, 2026',
      minet: '09 min read',
      title: 'The Impact of AI and Machine Learning on IT Solutions',
      slug: '15',
   },
   {
      id: 16,
      gridPage: blog_14_3,
      grid: grid1,
      date: 'February 14, 2026',
      minet: '10 min read',
      title: 'The Impact of AI and Machine Learning on IT Solutions',
      slug: '16',
   }


]

export default BlogData;